...
function newDraft(sourceText = ""): PendingInventoryDraft {
  return { status: "DRAFT", sourceText, fields: emptyDraftFields(), askedGaps: [], skippedGaps: [] };
}
...
export function inventoryDraftSnapshot(draft: PendingInventoryDraft) {
  ...
  return { status: draft.status, fields: draft.fields, canSave: hasInventoryIdentity(draft.fields), missingIdentity };
}
...
export function applyInventoryDraftFacts(params: {
  ...
  return { conversation, draft, snapshot: inventoryDraftSnapshot(draft), acceptedFields };
}
...
export function prepareInventoryDraftConfirmation(
  ...
  const confirmed: PendingInventoryDraft = { ...draft, status: "WAITING_CONFIRMATION" };
  return {
    ...conversation,
