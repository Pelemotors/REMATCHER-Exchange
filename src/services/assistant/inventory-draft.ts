...
export type InventoryGapId =
  ...
  | "engine_displacement" | "dealer_price" | "ownership" | "trim" | "color";
...
export interface InventoryDraftFields {
  ...
  /** Legacy storage alias. Product semantics use one asking price. */
...
export function emptyDraftFields(): InventoryDraftFields {
  return {
    make: null, model: null, trim: null, year: null, mileage: null, color: null,
    ownershipHand: null, ownershipType: null, retailPrice: null, b2bPrice: null,
    region: null, fuelType: null, engineDisplacementCc: null, features: [],
  };
}
...
function askingPrice(fields: InventoryDraftFields): number | null {
  return fields.b2bPrice ?? fields.retailPrice ?? null;
}
...
export function normalizeAskedGaps(gaps: Array<InventoryGapId | "b2b_price" | string>): InventoryGapId[] {
...
function hasDealerPrice(fields: InventoryDraftFields): boolean {
  return askingPrice(fields) != null;
}
...
export function isCommerciallyComplete(draft: PendingInventoryDraft): boolean {
  ...
  const engineOk = !engineIsApplicable(f) || f.engineDisplacementCc != null || wasResolved(draft, "engine_displacement");
...
export function openCommercialGaps(draft: PendingInventoryDraft): InventoryGapId[] {
  ...
  if (engineIsApplicable(f) && f.engineDisplacementCc == null && !wasResolved(draft, "engine_displacement")) gaps.push("engine_displacement");
...
export function nextGapToAsk(draft: PendingInventoryDraft): InventoryGapId | null {
  return isCommerciallyComplete(draft) ? null : openCommercialGaps(draft)[0] ?? null;
}
...
export function gapQuestion(gap: InventoryGapId, fields?: InventoryDraftFields): string {
  switch (gap) {
    ...
    case "engine_displacement": return "מה נפח המנוע? אפשר לכתוב למשל 1600 או 1.6 ליטר.";
    case "dealer_price": return "מה המחיר המבוקש על הרכב? אפשר להמשיך בלי כרגע, אבל לא תיווצר התאמה סופית עד שיוגדר מחיר.";
...
/**
 * @deprecated Semantic ownership interpretation belongs to the AI agent/Exchange Brain.
 * Kept only for compatibility with old callers; it intentionally does not interpret free text.
 */
export function parseOwnershipAnswer(_message: string): Partial<InventoryDraftFields> | "skip" | null {
  return null;
}
...
/** Legacy direct-gap parser: numeric/literal values only; semantic text is left for AI. */
export function parseGapAnswer(gap: InventoryGapId, message: string): Partial<InventoryDraftFields> | "skip" | null {
  ...
  if (gap === "dealer_price") {
    ...
    return n != null ? { b2bPrice: n, retailPrice: n } : null;
  }
  if (gap === "year" && /^\s*(20\d{2}|\d{2})\s*$/.test(m)) {
    let year = parseInt(m, 10);
    if (year < 100) year += 2000;
    return year >= 1990 && year <= 2035 ? { year } : null;
  }
  if (gap === "engine_displacement" && /^\s*\d{3,4}\s*$/.test(m)) {
    const n = Number(m.trim());
    return n >= 300 && n <= 10000 ? { engineDisplacementCc: n } : null;
  }
...
/** @deprecated Amendments are interpreted by the AI agent via update_inventory_draft. */
export function parseAmendment(_message: string): Partial<InventoryDraftFields> | null {
  return null;
}
...
export function buildStructuredSummary(draft: PendingInventoryDraft): string {
  const f = draft.fields;
  const lines = [`${[f.make, f.model, f.trim].filter(Boolean).join(" ") || "רכב"} ${f.year ?? "שנה לא ידועה"}`.trim()];
...
export function buildCompactSummary(draft: PendingInventoryDraft): string {
  const f = draft.fields;
  const bits = [`${[f.make, f.model].filter(Boolean).join(" ") || "רכב"} ${f.year ?? ""}`.trim()];
...
export function canConfirm(draft: PendingInventoryDraft): boolean { return hasInventoryIdentity(draft.fields); }
...
export function applyFields(draft: PendingInventoryDraft, patch: Partial<InventoryDraftFields>): PendingInventoryDraft {
  ...
  if ("b2bPrice" in patch || "retailPrice" in patch) {
    const price = patch.b2bPrice ?? patch.retailPrice ?? null;
    merged.b2bPrice = price;
    merged.retailPrice = price;
  }
...
export function markGapSkipped(draft: PendingInventoryDraft, gap: InventoryGapId): PendingInventoryDraft {
  ...
  return skipped.includes(gap) ? withAsked : { ...withAsked, skippedGaps: [...skipped, gap] };
}
export function advanceDraftAfterGap(draft: PendingInventoryDraft, gap: InventoryGapId, result: Partial<InventoryDraftFields> | "skip"): PendingInventoryDraft {
  return result === "skip" ? markGapSkipped(draft, gap) : applyFields(markGapAsked(draft, gap), result);
}
...
export function identityPartialMessage(fields: InventoryDraftFields): string {
  const known: string[] = [];
  const name = [fields.make, fields.model].filter(Boolean).join(" "); if (name) known.push(name);
