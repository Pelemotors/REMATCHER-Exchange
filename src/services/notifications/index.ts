...
export async function createNotification(params:{userId:string;type:NotificationType;title:string;body:string;link?:string;entityType?:string;entityId?:string;sendPush?:boolean;sourceCategory?:NotificationSourceCategory;pushSource?:PushSource;pushTriggerType?:PushTriggerType;dealerId?:string}) {
  // A CandidateMatch is a durable business object. Re-evaluation/rematch must never
  // create a second "new match" push for the same user/candidate.
  ...
  if(params.sendPush!==false){
    const prefs=await prisma.notificationPreference.findUnique({where:{userId:params.userId}}); const category=params.sourceCategory??"PRODUCT";
